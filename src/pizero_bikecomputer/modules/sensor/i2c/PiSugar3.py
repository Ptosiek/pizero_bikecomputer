...
class PiSugar3(i2c.i2c):
    ...
    def read(self):
        ...
        v = self.bus.read_i2c_block_data(
            ...
            self.BATTERY_VOLTAGE_BYTES,
        )
        self.battery_voltage = ((v[0] << 8) + v[1]) / 1000

        self.battery_level_array[:-1] = self.battery_level_array[1:]
        self.battery_level_array[-1] = self.get_battery_level_from_curve(
            self.battery_voltage
        )

        self.values["battery_level"] = int(
            round(np.nanmean(self.battery_level_array), 0)
        )
