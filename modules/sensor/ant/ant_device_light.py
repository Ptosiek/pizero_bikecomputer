...
class ANT_Device_Light(ANT_Device):
        ...
        for data in iter(self.send_queue.get, None):
            try:
                if not self.channel.send_acknowledged_data_with_retry(data):
                    app_logger.error(
                        f"ANT+ light acknowledged_data retry error: {self.format_list(data)}"
                    )
            except Exception as e:
                app_logger.error(f"{e}")
                app_logger.error(
                    f"ANT+ light acknowledged_data timeout: {self.format_list(data)}"
                )
    ...
    def on_data(self, data):
        if data[0] == 0x01:
            ...
            seq_no = data[4]
            time_delta = (
                datetime.now() - self.values["changed_timestamp"]
            ).total_seconds()
    ...
    def change_light_setting_off(self, auto=False, auto_id=None):
        if auto:
            ...
            if any(self.auto_off_status.values()) or t < self.auto_light_min_duration:
                return
