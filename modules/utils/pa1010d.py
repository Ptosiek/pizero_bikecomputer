        ...
        while time.time() < timeout:
            ...
            # ProprietarySentence handles boot up output such as "$PMTK011,MTKGPS*08"
                ...
                try:
                    ...
                    pass
                raise RuntimeError(
                    "Unsupported message type {type} ({sentence})".format(
                        type=type(result), sentence=sentence
                    )
                )

        raise TimeoutError(
            "Timeout waiting for {wait_for} message.".format(wait_for=wait_for)
        )
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if result:
            print(
                f"Time:      {gps.timestamp}"
                f"DateTime:  {gps.datetimestamp}"
                f"Longitude: {gps.longitude: .5f} {gps.lon_dir}"
                f"Latitude:  {gps.latitude: .5f} {gps.lat_dir}"
                f"Altitude:  {gps.altitude}"
                f"Geoid_Sep: {gps.geo_sep}"
                f"Geoid_Alt: {float(gps.altitude) + -float(gps.geo_sep)}"
                f"Used Sats: {gps.num_sats}"
                f"Quality:   {gps.gps_qual}"
            )
        time.sleep(1.0)
